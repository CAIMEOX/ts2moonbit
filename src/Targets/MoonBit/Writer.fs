...
let header = str "///| Generated by Ts2MoonBit"
...
let setTypeOptions (ctx : IContext<Options>) =
...
let createStructuredText _ _ = failwith "createStructuredText: todo"
let handleExports _ = failwith "handleExports: todo"

let emitReferenceTypeDirectives _ =
  failwith "emitReferenceTypeDirectives: todo"

let emitReferenceFileDirectives _ =
  failwith "emitReferenceFileDirectives: todo"

let emitStatementsWithStructuredText _ =
  failwith "emitStatementsWithStructuredText: todo"

let private emitImpl (sources : SourceFile list) (info : PackageInfo option) (ctx : IContext<Options>) : Output =
  let trace = ctx.logger.tracef "* emitImpl: %s"
  let tracef = ctx.logger.tracef
  ...
  let derivedOutputFileName =
    ...
    |> Option.defaultWith (fun () -> "unknown.mbt")
  ...
  trace ("mergedFileName: " + mergedFileName)
  trace ("derivedOutputFileName: " + derivedOutputFileName)
  ...
  tracef "* emitting a binding to '%s' " derivedModuleName
  ...
  {
    fileName = derivedOutputFileName
    content = cont
    stubLines = []
  }

let emit (input : Input) (ctx : IContext<Options>) : Output list =
  input.sources |> List.map (fun source -> emitImpl [ source ] input.info ctx)
